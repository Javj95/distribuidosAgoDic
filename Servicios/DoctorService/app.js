const express = require('express');
const app = express();
const mongoose = require('mongoose');
const morgan = require('morgan');

app.use(morgan('dev'));

app.use(express.urlencoded({ extended: true }));//permite mandar datos a traves de un FORMULARIO
app.use(express.json());

// Permite el acceso a cualquier cliente, evita CORS errors
app.use((req, res, next) => {
    res.header('Access-Control-Allow-Origin', "*");
    res.header('Access-Control-Allow-Headers',
        'Origin, X-Requested-With, Content-Type, Accept, Authorization')
    if (req.method === 'OPTIONS') {
        res.header('Access-Control-Allow-Methods', 'POST, GET')
        return res.status(200).json({});
    }
    next();
});

//conectarse a la BD
mongoose.connect('mongodb://localhost/base1', {
    useNewUrlParser: true,
    useUnifiedTopology: true
})
.then(
    db => console.log('Conectado a la bd')
).catch(err => console.log("Error al conectarse:" + err));


//importar rutas
const indexRutas = require('./routes/index');

app.use('/', indexRutas);

// Al no utilizar rutas disponibles con una solicitud, mandamos error 404
app.use((req, res, next) => {
    const error = new Error("Not found");
    error.status = 404;
    next(error);
})

app.use((error, req, res, next) => {
    res.status(error.status || 500);
    res.json({
        error: {
            message: error.message
        }
    });
});

app.listen(3000, () => {
    console.log("Express desde puerto 3000");
});